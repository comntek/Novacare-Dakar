...
import {
  ...
  getPatientsByMedecin, marquerCommeLu,
...
function ModalNouvelleConv({ onClose, onStart, utilisateurs, patients }) {
  const [recherche,   setRecherche]   = useState('')
  const [selectionne, setSelectionne] = useState(null)
  const [chargement,  setChargement]  = useState(false)

  const contacts = [
    ...utilisateurs.filter((u) => ['medecin', 'secretaire', 'admin'].includes(u.role)),
    ...patients.map((p) => ({
      id: p.id, prenom: p.prenom, nom: p.nom, role: 'patient',
    })),
  ]
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content max-w-sm" onClick={(e) => e.stopPropagation()}>
              ...
              contactsFiltres.map((c) => (
                <button key={c.id} onClick={() => setSelectionne(c)}
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-neutral-muted capitalize">{c.role}</p>
                  </div>
                </button>
              ))
            )}
        ...
        <div className="modal-footer justify-end gap-3">
          ...
          <button onClick={handleStart} disabled={!selectionne || chargement} className="btn-primary">
            {chargement ? <><Loader2 className="w-4 h-4 animate-spin" /> Ouverture...</> : 'Démarrer'}
          </button>
        </div>
      </div>
    </div>
...
export function MedecinMessagerie() {
  ...
  const chargerConversations = async () => {
    ...
    setErreur(null)
    try {
      const [convs, utils, pats] = await Promise.all([
        getConversations(user.uid),
        getUtilisateurs(),
        getPatientsByMedecin(user.uid),
      ])
      setConversations(convs)
      setUtilisateurs(utils.filter((u) =>
        u.id !== user.uid && ['admin', 'medecin', 'secretaire'].includes(u.role)
      ))
      setPatients(pats)
    } catch (e) {
  ...
  const handleEnvoyer = async () => {
    ...
    try {
      await envoyerMessage(convActive.id, {
        ...
        expediteurNom:  `Dr. ${user.prenom} ${user.nom}`,
        texte:          texte.trim(),
      ...
      <div className={`w-full sm:w-72 flex-shrink-0 card p-0 flex flex-col overflow-hidden
        ...
        <div className="p-4 border-b border-neutral-border space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setModalNouvelle(true)} className="btn-icon">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {chargement ? (
            ...
            <div className="empty-state py-12">
              ...
              <button onClick={() => setModalNouvelle(true)} className="btn-outline btn-sm mt-3">
      ...
      {convActive ? (
        ...
        <div className="hidden sm:flex flex-1 card items-center justify-center">
          <div className="empty-state">
            ...
            <button onClick={() => setModalNouvelle(true)} className="btn-primary mt-4">
...
export default MedecinMessagerie