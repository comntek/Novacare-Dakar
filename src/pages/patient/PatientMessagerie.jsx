...
import {
  ...
  Loader2, MessageSquare, Plus, Info,
} from 'lucide-react'
import { useAuthStore } from '../../store/authStore'
import {
  getConversations, ecouterMessages, envoyerMessage,
  creerOuOuvrirConversation, getPatientById,
  getSecretaires, getUtilisateurById, marquerCommeLu,
...
function ModalNouvelleConv({ medecinReferent, secretaires, onClose, onStart }) {
  const [selectionne, setSelectionne] = useState(null)
  const [envoi,       setEnvoi]       = useState(false)

  const contacts = [
    ...(medecinReferent ? [{
      id: medecinReferent.id,
      prenom: medecinReferent.prenom,
      nom: medecinReferent.nom,
      role: 'medecin',
      tag: 'Votre médecin référent',
    }] : []),
    ...secretaires.map((s) => ({
      id: s.id, prenom: s.prenom, nom: s.nom,
      role: 'secretaire', tag: 'Secrétariat',
    })),
  ]
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body space-y-4">
          <div className="p-3 bg-info-50 border border-info-100 rounded-xl flex gap-2">
            <Info className="w-4 h-4 text-info flex-shrink-0 mt-0.5" />
            <p className="text-xs text-neutral-muted">
              Vous pouvez contacter votre médecin référent et le secrétariat uniquement.
            </p>
          </div>
          ...
          {contacts.length === 0 ? (
            <div className="text-center py-6">
              ...
              <p className="text-sm font-medium text-neutral-text">Aucun contact disponible</p>
              <p className="text-xs text-neutral-muted mt-1">
                Aucun médecin référent assigné. Contactez le secrétariat.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {contacts.map((c) => (
                <button key={c.id} onClick={() => setSelectionne(c)}
                  ...
                  <div className="text-left flex-1 min-w-0">
                    <p className="text-sm font-semibold text-neutral-text">
                      {c.role === 'medecin' ? 'Dr. ' : ''}{c.prenom} {c.nom}
                    </p>
                    <p className="text-xs text-neutral-muted">{c.tag}</p>
                  </div>
                  {selectionne?.id === c.id && (
...
export function PatientMessagerie() {
  ...
  const [medecinRef,    setMedecinRef]    = useState(null)
  ...
  const chargerConversations = async () => {
    ...
    setErreur(null)
    try {
      const [convs, patient, secs] = await Promise.all([
      ...
      setSecretaires(secs)
      if (patient?.medecinReferentId) {
        try {
          const med = await getUtilisateurById(patient.medecinReferentId)
          setMedecinRef(med)
        } catch (e) { setMedecinRef(null) }
      }
    } catch (e) {
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-neutral-border">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-muted capitalize">
                {convActive.roleInterlocuteur === 'medecin' ? 'Votre médecin référent'
                  : convActive.roleInterlocuteur === 'secretaire' ? 'Secrétariat NovaCare'
